...
## 它给 OpenCode 带来的提升 / 好处

OpenCode 的会话会越聊越长。一个会话几十轮之后，想找「我之前问过的那句话」只能一路滚动；而左侧侧栏又只显示会话标题，**完全看不出每个会话里聊了什么**。这个插件正是补上这块短板：

- **一眼找到自己问过的话。** 不用在长长的对话里滚来滚去，hover 一下会话，就能在浮层里按顺序看到你发过的每一条消息。
- **一键精准跳转到那条消息。** 点击某条 → 直接跳过去并滚动定位，**哪怕它埋在虚拟列表深处**（复用官方的 reveal + hash 滚动机制，会自动把没渲染的目标渲染出来）。
- **不切会话就能预览。** 把鼠标停在侧栏任意会话上，就能回忆「这个会话当时在聊啥」，不用离开当前会话、不打断手头工作。
- **跨会话快速找回上下文。** 标题往往很笼统，**你自己发的消息才是真正的索引**。浮层把每个会话变成一份「你提过的问题」的可点击目录。
- **长会话工作流的效率利器。** 大型重构、多步调试、需要反复回看早先指令的研究型会话，尤其好用。
- **这是把它找回来的唯一方式。** 该功能官方上线过又被移除；本插件在**不改 App、不破坏签名、无需重新编译**的前提下把它加回来，且能在官方大多数更新后继续可用。
- **零长期负担。** 没有后台臃肿、没有 npm 依赖树、不需要账号。助手只在 OpenCode 开着时运行，退出即自动停止。

一句话：它把侧栏从「一串看不懂的标题」变成了**一份可搜索、可一键跳转的「你问过的一切」的地图**——省下滚动，也省下「我到底在哪说过这句」的苦苦翻找。
...
核心（`inject.js`、`hover-helper.js`）与系统无关，且 `opencode.db` 路径会**自动探测**，所以只有「启动器」因平台而异。

### Windows

已附带 PowerShell 启动器 **`start-opencode-hover.ps1`**（脚本内全英文，避免 Windows 控制台中文乱码），逻辑与 macOS 版一致：停旧助手 → 带 `--remote-debugging-port=9222` 启动 OpenCode → 等端口就绪 → 挂助手。
...
脚本会自动探测 `OpenCode.exe`（Scoop 的 `opencode-desktop`，或 `%LOCALAPPDATA%\Programs\...`）。找不到就手动指定：

```powershell
$env:OPENCODE_EXE = "C:\path\to\OpenCode.exe"
```

想要全局快捷键：用 AutoHotkey / PowerToys / 开始菜单快捷方式 指向同一条命令即可（加 `-WindowStyle Hidden` 可静默）。

> 为什么 db 在 Windows 上也能找到：OpenCode 用 `xdg-basedir`，它在 Windows 上回退到 `~/.local/share`（即 `C:\Users\<你>\.local\share\opencode\opencode.db`），**而不是** `%APPDATA%`。助手会自动探测（可用 `OPENCODE_DB` 覆盖）。

### Linux

macOS 启动器是普通 bash，仅 `open -a` / `osascript` 是 macOS 专属。Linux 上自己带端口启动 OpenCode、再跑助手即可：

```bash
opencode-desktop --remote-debugging-port=9222 &   # 换成你安装的可执行文件
bun ~/.local/share/opencode/hover-plugin/hover-helper.js
```

db 路径（`$XDG_DATA_HOME/opencode` → `~/.local/share/opencode`）自动探测。
...
| 文件 | 作用 | 必需 |
|---|---|---|
| `start-opencode-hover.command` | macOS 启动器：停旧助手 → 带调试端口起 OpenCode → 挂助手 | macOS |
| `start-opencode-hover.ps1` | Windows 启动器（PowerShell，逻辑同 macOS 版，脚本全英文） | Windows |
...
### db 路径怎么跨平台自动找到的
`hover-helper.js` 按 OpenCode 自身的规则（`xdg-basedir`）定位 db，无任何写死路径：
1. 若设了环境变量 **`OPENCODE_DB`** → 直接用（支持 `:memory:`、绝对路径、或相对 data 目录的文件名）。
2. 否则 data 目录 = **`$XDG_DATA_HOME/opencode`**，未设则回退 **`~/.local/share/opencode`**（macOS / Linux / **Windows** 一致，Windows 也不是 `%APPDATA%`）。
3. 目录内优先用 `opencode.db`，否则挑最近修改的 `opencode-<channel>.db`（dev/beta 版会带渠道后缀）。`$HOME` 用 `os.homedir()` 取，避免 Windows 下 `process.env.HOME` 为空。
